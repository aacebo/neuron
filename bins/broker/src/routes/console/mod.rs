...
async fn page(ctx: RequestContext) -> error::Result<HttpResponse> {
    let tenant_id = console_tenant_id(&ctx)?;
...
async fn connect(
    ...
    rt::spawn(run_stream(ctx, tenant_id, cursor, session, stream, notifications));
    Ok(response)
}
        ...
        tokio::select! {
            message = stream.next() => {
                match message {
                    ...
                    Some(Ok(AggregatedMessage::Close(_))) | Some(Err(_)) | None => return,
...
async fn send_event(session: &mut actix_ws::Session, event: &types::events::Event) -> error::Result<()> {
    session.text(serde_json::to_string(event)?).await.map_err(error::http)
}

fn console_tenant_id(ctx: &RequestContext) -> error::Result<uuid::Uuid> {
    ctx.console()
        .tenant_id
        .ok_or_else(|| error::config("console tenant is not configured"))
}
