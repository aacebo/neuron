use rust_bert::pipelines::token_classification::{Token, TokenClassificationModel};

use crate::{Action, types};

pub struct PII<'a> {
    model: &'a TokenClassificationModel,
}

impl<'a> PII<'a> {
    pub fn new(model: &'a TokenClassificationModel) -> Self {
        Self { model }
    }
}

impl<'a> Action for PII<'a> {
    fn name(&self) -> &'static str {
        "pii::extract"
    }

    fn invoke(&self, ctx: &mut crate::Context) -> Result<(), Box<dyn std::error::Error>> {
        let out = self.model.predict(ctx.input, true, false);

        for sequence in out {
            let mut entities = PiiEntities::new(ctx.options.min_score as f64);

            for entity in sequence {
                entities.push(entity);
            }

            ctx.annotations.extend(entities.finish());
        }
...
struct PiiEntity {
    annotation: types::CortexAnnotation,
    token_count: usize,
}
